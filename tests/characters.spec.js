...
test("monogram styles preserve accented initials and explain fallback lettering", async ({ page }) => {
  await page.getByRole("button", { name: "Monogram Maker", exact: false }).click();
  const dialog = page.getByRole("dialog", { name: "Monogram Maker" });
  await dialog.getByRole("button", { name: /Optima/ }).click();
  await dialog.getByLabel("Monogram font style", { exact: true }).selectOption("bold");
  await dialog.getByRole("textbox", { name: "Left initial", exact: true }).fill("\u00e9");
  await dialog.getByRole("textbox", { name: "Center initial (larger in classic style)", exact: true }).fill("B");

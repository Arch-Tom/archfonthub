...
export default function MonogramMaker({ fontLibrary, onClose, onInsert }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1E284B]/[0.82] backdrop-blur-[3px] p-0 sm:p-2 animate-fade-in" onClick={onClose}>
            <div className="relative border border-[#BFD5E8] bg-[#FFFFFF] rounded-none sm:rounded-3xl shadow-2xl w-full max-w-full sm:max-w-2xl md:max-w-3xl h-screen sm:h-[90vh] flex flex-col overflow-hidden transition-all" onClick={e => e.stopPropagation()}>
                <header className="sticky top-0 z-10 bg-[linear-gradient(135deg,#1E284B,#3677B3)] border-b border-white/20 flex justify-between items-center px-4 sm:px-5 py-3 sm:py-4 text-white">
                    <div>
                        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white tracking-tight" style={{ fontFamily: 'Alumni Sans Regular' }}>Monogram Maker</h2>
                        <p className="text-[#E7EAE7] text-xs sm:text-sm">Create your monogram instantly.</p>
                    </div>
                    <button className="text-3xl font-light text-white/85 hover:text-white transition p-2 -mr-2" onClick={onClose} title="Close">&times;</button>
                </header>
                ...
                <div className="flex-1 flex flex-col overflow-y-auto pb-[100px] items-center px-4 sm:px-6">
                    <div className="w-full max-w-xl flex flex-col gap-4 mt-6">
                        ...
                        <div className="flex justify-center gap-2 bg-[#EAF5FC] p-1 rounded-xl shadow-inner overflow-x-auto">
                            {['classic', 'flat', 'circular', 'split'].map(style => (
                                    <button key={style} onClick={() => setMonogramStyle(style)} className={`px-4 py-2 rounded-lg font-semibold text-sm transition capitalize ${monogramStyle === style ? 'bg-[#478CCA] text-white shadow' : 'border border-[#BFD5E8] bg-[#FFFFFF] text-[#478CCA] hover:bg-[#EAF5FC]'}`}>
                        ...
                        {monogramStyle === 'split' ? (
                            <div className="flex flex-col items-center gap-3 mt-4">
                                <h3 className="text-base sm:text-lg font-semibold text-[#478CCA] mb-1 text-center">Initial & Name</h3>
                                <div className="flex items-center justify-center gap-4">
                                    <input ref={splitInitialInputRef} type="text" placeholder="S" value={splitInitial} onChange={e => setSplitInitial(e.target.value.slice(0, 1).toUpperCase())} maxLength={1} className="w-16 h-16 bg-[#FFFFFF] border-2 border-[#BFD5E8] text-4xl text-center rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#EAF5FC] focus:border-[#478CCA] transition" />
                                    <input ref={splitNameInputRef} type="text" placeholder="SMITH" value={splitName} onChange={e => setSplitName(e.target.value.toUpperCase())} className="h-16 px-4 bg-[#FFFFFF] border-2 border-[#BFD5E8] text-2xl text-center rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#EAF5FC] focus:border-[#478CCA] transition w-48" />
                                </div>
                            </div>
                        ) : (
                            <div>
                                <h3 className="text-base sm:text-lg font-semibold text-[#478CCA] mb-2 sm:mb-3 text-center">Your Initials</h3>
                                <div className="flex justify-center gap-3">
                                    {initials.map((initial, index) => (
                                        <input key={index} ref={el => (inputRefs.current[index] = el)} type="text" placeholder={['N', 'X', 'D'][index]} value={initial} onChange={e => handleInitialChange(e, index)} onKeyDown={e => handleKeyDown(e, index)} maxLength={1} className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-[#FFFFFF] border-2 border-[#BFD5E8] text-2xl sm:text-3xl md:text-4xl text-center rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#EAF5FC] focus:border-[#478CCA] transition" />
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    {monogramStyle === 'circular' && (
                        <div className="w-full max-w-xl mt-6">
                            <h3 className="text-base sm:text-lg font-semibold text-[#478CCA] mb-2 sm:mb-3 text-center">Frame Style</h3>
                            <div className="grid grid-cols-3 gap-2 bg-[#EAF5FC] p-2 rounded-xl shadow-inner">
                                {['none', 'solid', 'double', 'dotted', 'outline', 'thick-thin'].map(style => (
                                    <button key={style} onClick={() => setFrameStyle(style)} className={`px-4 py-2 rounded-lg font-semibold text-sm capitalize transition w-full ${frameStyle === style ? 'bg-[#478CCA] text-white shadow' : 'border border-[#BFD5E8] bg-[#FFFFFF] text-[#478CCA] hover:bg-[#EAF5FC]'}`}>
                                        {style.replace('-', ' & ')}
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className="w-full max-w-xl mt-6">
                        <div className="w-full flex flex-col items-center mb-2">
                            <label className="text-xs font-semibold text-[#4D5B68] mb-1" htmlFor="monogram-preview-box">Preview</label>
                            <div id="monogram-preview-box" className="w-full max-w-[400px] flex items-center justify-center rounded-xl border border-[#BFD5E8] shadow-inner bg-[#FFFFFF] px-2" style={{ height: '180px', minHeight: '180px' }}>
                                {monogramStyle === 'split' ? (
                    ...
                    {monogramStyle !== 'circular' && (
                        <div className="w-full max-w-2xl mt-6">
                            <h3 className="text-base sm:text-lg font-semibold text-[#478CCA] mb-2 sm:mb-3 text-center">Choose a Font {monogramStyle === 'split' && 'for the Initial'}</h3>
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                {visibleFonts.map(font => (
                                    <button key={font.name} onClick={() => { setSelectedFont(font); setActiveStyle(Object.keys(font.styles)[0]); }} className={`group flex flex-col items-center justify-between rounded-xl border-2 transition h-32 w-full p-3 ${selectedFont.name === font.name ? 'border-[#478CCA] bg-[#EAF5FC] shadow-md' : 'border-[#BFD5E8] bg-[#FFFFFF] hover:border-[#478CCA] hover:bg-[#EAF5FC]'}`}>
                                        ...
                                        <span className="text-xs font-semibold text-[#478CCA] group-hover:text-[#1E284B] text-center">{font.name}</span>
                                        <span className="text-[11px] text-[#58A068]">{font.category}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
                ...
                <footer className="fixed sm:sticky bottom-0 left-0 w-full bg-[#FFFFFF]/95 border-t border-[#BFD5E8] px-4 py-3 flex flex-row-reverse justify-between gap-3 z-20">
                    <button onClick={handleInsert} disabled={isInsertDisabled()} className="px-8 py-3 bg-[#478CCA] text-white rounded-2xl hover:bg-[#3677B3] font-bold transition-colors shadow-sm text-base disabled:opacity-50 disabled:cursor-not-allowed">Insert Monogram</button>
                    <button onClick={onClose} className="px-6 py-3 bg-[#EAF5FC] text-[#478CCA] rounded-2xl hover:bg-[#EAF5FC] font-semibold transition-colors shadow-sm text-base">Cancel</button>
                </footer>
            </div>
        </div>
    );

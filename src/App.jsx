        ...
        {(eyebrow || title || description || action) && (
          <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-[48rem]">
              ...
              {description && (
                <p className={`mt-2 text-[15px] leading-7 ${descriptionClasses}`}>{description}</p>
              )}
            </div>
...
const StatusPill = ({ children, tone = 'slate' }) => {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold shadow-sm ${tones[tone] || tones.slate}`}>
      {children}
    </span>
  );
};
...
const App = () => {
  ...
  const selectedFontNames = useMemo(() => selectedFonts.map((font) => font.name), [selectedFonts]);
  ...
  const handleLineStyleChange = (lineIndex, styleKey) => {
    setLineSettings((prevSettings) =>
      prevSettings.map((line, index) => (index === lineIndex ? { ...line, styleKey } : line))
    );
  };
    ...
    if (hasStandardSelection) {
      ...
      for (const [index, line] of populatedPreviewLines.entries()) {
        ...
        const activeFontFamily =
          font?.styles[line.styleKey] || font?.styles[getDefaultStyleKey(font?.name)] || 'inherit';
                ...
                action={
                  <div className="flex flex-wrap gap-2">
                    ...
                    <StatusPill tone={textLineCount > 0 ? 'emerald' : 'slate'}>
                      {textLineCount > 0
                        ? `${textLineCount} preview line${textLineCount === 1 ? '' : 's'}`
                        : 'No preview text'}
                    </StatusPill>
                    ...
                    <div className="mt-4 flex min-h-[58px] flex-wrap gap-2.5">
                      {selectedFontNames.length > 0 ? (
                        selectedFontNames.map((name, index) => {
                          ...
                          return (
                            <StatusPill key={name} tone={tones[index % tones.length]}>
                              {name}
                            </StatusPill>
                          );
                        })
                  ...
                  <div className="flex justify-center gap-1.5">
                    <button
                      ...
                      className={`h-12 w-24 flex items-center justify-center rounded-lg text-slate-800 text-lg font-semibold shadow-sm transition-colors ${
                        isShifted ? 'bg-blue-500 text-white' : 'bg-white hover:bg-blue-100'
                      }`}
                    >
                      Shift
                    </button>
